...
export default function Products(props) {
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        ...
        if (response.ok) {
          ...
          console.log(response,'Manish products');
    ...
    async function fetchCategories() {
      try {
        ...
        response = await response.json();
        console.log(response,'Manish categories');
  ...
  //   console.log("mnaish cart");
  //   // let response = await fetch("http://127.0.0.1:5000/addToCart");
  //   // response = await response.json();
  //   setAddToCart('Product added to cart successfully')
